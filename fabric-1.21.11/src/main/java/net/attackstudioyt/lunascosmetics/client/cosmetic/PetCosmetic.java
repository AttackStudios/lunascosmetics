...
public class PetCosmetic extends Cosmetic {
    ...
    public PetCosmetic(String id, String name, String description, int accent, Kind kind,
        ...
        this.texture = Identifier.of(LunasCosmetics.MOD_ID, "textures/cosmetic/" + folder + "/" + texName + ".png");
        this.blinkTexture = Identifier.of(LunasCosmetics.MOD_ID, "textures/cosmetic/" + folder + "/" + texName + "_blink.png");
        this.glowTexture = glow ? Identifier.of(LunasCosmetics.MOD_ID, "textures/cosmetic/" + folder + "/" + texName + "_glow.png") : null;
    ...
    public void render(RenderCtx ctx) {
        ...
        Identifier tex = b.eyesClosed() ? blinkTexture : texture;
        ctx.sink().model(model, pose, tex, -1);
        if (glowTexture != null) {
            int a = 255;
            if (twinkle) {
                a = (int) (175 + 80 * (0.5f + 0.5f * MathHelper.sin(b.time * 2.3f)));
            }
    ...
    private static void poseCatLoaf(LunaModel.Pose p, Brain b, Inputs in, boolean gui) {
        float t = b.time;
        float sleep = b.sleep;
        float breathe = MathHelper.sin(t * (sleep > 0.5f ? 1.3f : 2.1f)) * (0.025f + 0.02f * sleep);
        float puff = 1 + b.flinch() * 0.1f;

        // Keep the cat roughly level when the wearer looks up or down: it grips on.
        float balance = gui ? 0 : MathHelper.clamp(-in.headPitch(), -60, 60) * 0.7f;
        float sway = MathHelper.sin(in.walkPhase() * 0.6662f) * 4f * in.moving();
        ...
        float kL = knead * MathHelper.sin(t * 9) * 14;
        float kR = knead * MathHelper.sin(t * 9 + MathHelper.PI) * 14;
        ...
        float swish = MathHelper.sin(b.tailPhase) * amp * (1 - sleep);
        float follow = MathHelper.sin(b.tailPhase - 0.9f) * (amp + 10) * (1 - sleep);
    ...
    private static void poseCatSitting(LunaModel.Pose p, Brain b, Inputs in) {
        float t = b.time;
        float breathe = MathHelper.sin(t * 2.1f) * 0.025f;
        float sway = MathHelper.sin(in.walkPhase() * 0.6662f) * 5f * in.moving();
        ...
        p.rotDeg("ear_right", flat, 0, b.earRight * 28);
        float knead = b.kneading();
        p.rotDeg("leg_front_left", 60 + knead * MathHelper.sin(t * 9) * 10, 0, 0);
        p.rotDeg("leg_front_right", 60 + knead * MathHelper.sin(t * 9 + MathHelper.PI) * 10, 0, 0);
        ...
        float swish = MathHelper.sin(b.tailPhase) * (18 + in.moving() * 14);
        p.rotDeg("tail", -50, 25 + swish, 0);
        p.rotDeg("tail_tip", -30, 30 + MathHelper.sin(b.tailPhase - 0.9f) * 26, 0);
    }
    ...
    private static void poseMoosh(LunaModel.Pose p, Brain b, Inputs in) {
        float t = b.time;
        float sleep = b.sleep;
        float breathe = MathHelper.sin(t * 2.4f) * 0.03f;
        float hop = b.bounce();
        float hopY = hop > 0 ? -MathHelper.sin(hop * MathHelper.PI) * 2.2f : 0;
        float squash = hop > 0 ? MathHelper.sin(hop * MathHelper.PI * 2) * 0.08f : 0;
        float sway = MathHelper.sin(in.walkPhase() * 0.6662f) * 6f * in.moving();
        float wobble = MathHelper.sin(t * 1.7f) * 3;
        ...
        float flop = MathHelper.sin(t * 3.1f) * 8 + hop * 25 + in.moving() * MathHelper.sin(in.walkPhase() * 1.3f) * 14;
        ...
        float sproutSway = MathHelper.sin(t * 1.9f) * 9 + (hop > 0 ? MathHelper.sin(hop * 12) * 18 * (1 - hop) : 0);
        p.rotDeg("sprout", MathHelper.sin(t * 1.3f) * 4, 0, sproutSway);
        p.rotDeg("leaf_left", 0, 0, MathHelper.sin(t * 3.3f) * 7);
        p.rotDeg("leaf_right", 0, 0, -MathHelper.sin(t * 3.3f + 0.7f) * 7);

        // little legs: paddle when walking, kick on a hop
        float paddle = MathHelper.sin(in.walkPhase() * 0.6662f) * 20 * in.moving();
        ...
        p.rotDeg("tail", 25, MathHelper.sin(t * 9) * 35 * (1 - sleep), 0);
    }

    public static void applyYaw(MatrixStack m, float deg) {
        m.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(deg));
    }
}
