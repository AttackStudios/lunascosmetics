...
public final class BbModel {
    ...
    public static final class Mesh {
        ...
        public void emit(MatrixStack.Entry entry, VertexConsumer vc, int light) {
        ...
        private void vertex(MatrixStack.Entry entry, VertexConsumer vc, int p, int u, int n, int light) {
            vc.vertex(entry, positions[p] / 16.0f, positions[p + 1] / 16.0f, positions[p + 2] / 16.0f)
                    .color(255, 255, 255, 255)
                    .texture(uvs[u], uvs[u + 1])
                    .overlay(OverlayTexture.DEFAULT_UV)
                    .light(light)
                    .normal(entry, normals[n], normals[n + 1], normals[n + 2]);
        }
    }
    ...
    public static BbModel parse(String jsonText, String key) {
        ...
        if (root.has("textures")) {
            ...
            for (JsonElement el : root.getAsJsonArray("textures")) {
                ...
                Identifier id = Identifier.of(LunasCosmetics.MOD_ID, "dynamic/" + key + "_" + i++);
                try {
                    ...
                    MinecraftClient.getInstance().getTextureManager().registerTexture(id,
                            new NativeImageBackedTexture(() -> "lunascosmetics-" + key, img));
                } catch (Exception e) {
                    LunasCosmetics.LOGGER.warn("custom model texture failed: {}", e.toString());
                    id = Identifier.ofVanilla("textures/block/pink_wool.png");
                }
                model.textures.add(id);
            }
        }
        if (model.textures.isEmpty()) {
            model.textures.add(Identifier.ofVanilla("textures/block/pink_wool.png"));
