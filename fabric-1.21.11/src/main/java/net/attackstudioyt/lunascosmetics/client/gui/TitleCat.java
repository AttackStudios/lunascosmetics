...
public final class TitleCat {
    ...
    public void render(DrawContext ctx, int screenWidth, int mouseX, int mouseY) {
        ...
        float swish = MathHelper.sin(t * (asleep ? 0.8f : 2.2f)) * (asleep ? 0.12f : 0.45f);
        m.pushMatrix();
        m.translate(x + 15 * SCALE, y + 21 * SCALE);
        m.rotate(-0.4f + swish);
        m.scale(SCALE, SCALE);
        ...
        float breathe = MathHelper.sin(t * (asleep ? 1.2f : 2.3f)) * (asleep ? 0.035f : 0.02f);
        float hop = t - pokeAt < 0.35f ? MathHelper.sin((t - pokeAt) / 0.35f * MathHelper.PI) * 5 : 0;
        m.pushMatrix();
        m.translate(x + 12 * SCALE, y + 24 * SCALE - hop);
        m.scale(SCALE * (1 - breathe * 0.5f), SCALE * (1 + breathe));
        ...
        for (int i = fx.size() - 1; i >= 0; i--) {
            ...
            f[0] += f[2] * dt + (f[5] == 1 ? MathHelper.sin(f[4] * 3) * 6 * dt : 0);
            f[1] += f[3] * dt;
            float a = 1 - f[4] / life;
            if (f[5] == 1) {
                ...
                m.translate(f[0], f[1]);
                m.scale(s, s);
                ctx.drawText(MinecraftClient.getInstance().textRenderer, "z", 0, 0, Ui.alpha(0xFFFFE3EE, a), true);
                m.popMatrix();
            } else {
                f[3] += 30 * dt;
    ...
    public boolean click(double mx, double my) {
        if (mx >= x + 4 && mx <= x + 20 * SCALE && my >= y + 2 * SCALE && my <= y + 24 * SCALE) {
            pokeAt = t;
            idle = 0;
            Ui.play(SoundEvents.ENTITY_CAT_AMBIENT, 1.2f + (float) Math.random() * 0.4f, 0.7f);
            for (int i = 0; i < 5; i++) {
                float a = -MathHelper.PI / 2 + ((float) Math.random() - 0.5f) * 2;
                fx.add(new float[]{x + 12 * SCALE, y + 8 * SCALE, MathHelper.cos(a) * 25, MathHelper.sin(a) * 35, 0, 0});
            }
            return true;
        }
        return false;
