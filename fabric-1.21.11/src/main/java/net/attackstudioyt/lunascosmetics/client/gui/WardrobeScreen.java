...
public class WardrobeScreen extends Screen {
    ...
    public WardrobeScreen(Screen parent) {
        super(Text.translatable("lunascosmetics.title"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        relayField = new TextFieldWidget(textRenderer, 0, 0, 10, 18, Text.literal("Relay"));
        relayField.setMaxLength(200);
        relayField.setText(ClientConfig.get().relayUrl);
        relayField.setPlaceholder(Text.literal("wss://... (optional)"));
        relayField.setChangedListener(s -> {
            ClientConfig.get().relayUrl = s.trim();
            ClientConfig.save();
        });
        addSelectableChild(relayField);
        layoutTab();
    }
    ...
    private void layoutTab() {
        ...
        if (tab.slot != null) {
            ...
            buttons.add(new Btn("Open my folder", rightX1 + 8, by - 20, 100, 18,
                    () -> Util.getOperatingSystem().open(CustomLibrary.dir())));
    ...
    private void drawTitle(DrawContext ctx) {
        ...
        float s = 1.6f;
        float bob = MathHelper.sin(t * 2) * 1.2f;
        m.pushMatrix();
        m.translate(width / 2f - tw * s / 2, 8 + bob);
        m.scale(s, s);
        ctx.drawText(textRenderer, title, 0, 0, 0xFFFFE3EE, true);
        m.popMatrix();
        String face = ((int) (t * 10) % 37 == 0) ? "cat_cherry_blink" : "cat_cherry_open";
        Ui.sprite(ctx, face, (int) (width / 2f - tw * s / 2) - 22, 6, 16, 16);
        Ui.sprite(ctx, "icon_heart", (int) (width / 2f + tw * s / 2) + 6, 8 + (int) (MathHelper.sin(t * 3) * 2), 9, 9);
    }

    private void drawPreviewPanel(DrawContext ctx, int mouseX, int mouseY, float dt) {
        ...
        if (!dragging) {
            ...
            if (t - lastInteract > 3) {
                float target = 20 + MathHelper.sin(t * 0.4f) * 25;
                yaw += (target - yaw) * Math.min(1, dt * 0.8f);
            }
        }
        MinecraftClient client = MinecraftClient.getInstance();
        SkinTextures skin = client.player != null ? client.player.getSkin()
                : client.getSkinProvider().supplySkinTextures(client.getGameProfile(), false).get();
        int px1 = leftX1 + 6, px2 = leftX2 - 6, py1 = top + 6, py2 = bottom - 46;
        float scale = Math.min((py2 - py1) / 2.95f, (px2 - px1) / 1.3f);
        ctx.state.addSpecialElement(PreviewState.of(null, Wardrobe.mine(), "wardrobe-preview", yaw, pitch,
                skin.body().texturePath(), skin.model() == PlayerSkinType.SLIM, 1,
                px1, py1, px2, py2, scale, null));
        ...
        if (status.length() > 0 && client.getNetworkHandler() != null) {
            int w = textRenderer.getWidth(status);
            float sc = Math.min(1, (leftX2 - leftX1 - 10) / (float) w);
            Matrix3x2fStack m = ctx.getMatrices();
            m.pushMatrix();
            m.translate(leftX1 + 6, top + 6);
            m.scale(sc * 0.75f, sc * 0.75f);
            ctx.drawText(textRenderer, status, 0, 0, 0xFFFFC1D6, false);
            m.popMatrix();
        }
    }

    private boolean drawButton(DrawContext ctx, String label, int x, int y, int w, int h, int mx, int my) {
        boolean hover = mx >= x && mx < x + w && my >= y && my < y + h;
        Ui.panel(ctx, x, y, x + w, y + h, 4, hover ? 0xFFFFFFFF : 0xFFF7A1C0, hover ? 0xFFE0588A : 0xFFB83A66);
        ctx.drawCenteredTextWithShadow(textRenderer, Text.literal(label), x + w / 2, y + (h - 8) / 2, 0xFFFFFFFF);
        return hover;
    }
    ...
    private void drawRightPanel(DrawContext ctx, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < tabs.length; i++) {
            ...
            int lw = textRenderer.getWidth(tb.label);
            boolean showText = tabW > lw + 18;
            int contentW = showText ? 12 + lw : 12;
            int sx = x + (tabW - contentW) / 2;
            Ui.sprite(ctx, tb.icon, sx, ty + 3, 12, 12);
            if (showText) {
                ctx.drawText(textRenderer, tb.label, sx + 14, ty + 5, tb == tab ? 0xFFFFFFFF : 0xFFFFC1D6, true);
            }
        }
        ...
        ScreenRect clip = new ScreenRect(rightX1 + 2, gridY1, rightX2 - rightX1 - 4, gridY2 - gridY1);
        ctx.enableScissor(clip.getLeft(), clip.getTop(), clip.getRight(), clip.getBottom());
        Loadout mine = Wardrobe.mine();
        ...
        for (Card card : cards) {
            ...
            if (card.cosmetic != null) {
                ...
                ctx.state.addSpecialElement(PreviewState.of(card.cosmetic.id, mine, "card:" + card.cosmetic.id,
                        -25 + MathHelper.sin(spin * 0.017f) * 35 + (hover ? t * 40 % 360 : 0), 18, null, false, 1,
                        x + 2, y - lift + 4, x + CARD_W - 2, y - lift + 58, 34, clip));
            } else {
            ...
            if (selected) {
                float beat = 1 + MathHelper.sin(t * 5) * 0.08f;
                Matrix3x2fStack m = ctx.getMatrices();
                m.pushMatrix();
                m.translate(x + CARD_W - 6, y - lift + 6);
                m.scale(beat, beat);
        ...
        if (tab == Tab.MINE) {
            if (cards.isEmpty()) {
                ...
                ctx.drawCenteredTextWithShadow(textRenderer, Text.literal("Make your own cosmetics!"), cx, gridY1 + 20, 0xFFFFE3EE);
                ctx.drawCenteredTextWithShadow(textRenderer, Text.literal("Build one in Blockbench, save the .bbmodel"), cx, gridY1 + 36, 0xFFFFC1D6);
                ctx.drawCenteredTextWithShadow(textRenderer, Text.literal("into your folder, and it shows up here."), cx, gridY1 + 48, 0xFFFFC1D6);
            }
            for (Btn b : buttons) {
    ...
    private void drawFitted(DrawContext ctx, String s, int cx, int y, int maxW, int color) {
        int w = textRenderer.getWidth(s);
        if (w <= maxW) {
            ctx.drawText(textRenderer, s, cx - w / 2, y, color, true);
            return;
        }
        float sc = maxW / (float) w;
        Matrix3x2fStack m = ctx.getMatrices();
        m.pushMatrix();
        m.translate(cx - w * sc / 2, y + (1 - sc) * 4);
        m.scale(sc, sc);
        ctx.drawText(textRenderer, s, 0, 0, color, true);
        m.popMatrix();
    }

    private void drawSettings(DrawContext ctx, int mouseX, int mouseY) {
        for (Toggle tg : toggles) {
            ...
            ctx.drawText(textRenderer, tg.label, sx + 32, tg.y + 1, 0xFFFFFFFF, true);
    ...
    private void drawFittedLeft(DrawContext ctx, String s, int x, int y, int maxW, int color) {
        int w = textRenderer.getWidth(s);
        float sc = w <= maxW ? 0.85f : maxW / (float) w * 0.85f;
        Matrix3x2fStack m = ctx.getMatrices();
        m.pushMatrix();
        m.translate(x, y);
        m.scale(Math.min(sc, 0.85f), Math.min(sc, 0.85f));
        ctx.drawText(textRenderer, s, 0, 0, color, false);
        m.popMatrix();
    }
    ...
    public boolean mouseClicked(Click click, boolean doubled) {
        ...
        if (my >= ty && my < ty + 18 && mx >= rightX1 + 6 && mx < rightX1 + 6 + tabW * tabs.length) {
            ...
            if (picked != tab) {
                ...
                Ui.play(SoundEvents.ENTITY_CAT_AMBIENT, 1.8f, 0.25f);
                layoutTab();
            }
            return true;
        }
        ...
        if (mx >= leftX1 + 6 && mx < leftX2 - 6) {
            if (mine.get(Slot.PET) != null && my >= by && my < by + 17) {
                ...
                Ui.play(SoundEvents.ENTITY_CAT_PURREOW, 1.3f, 0.35f);
                return true;
            }
            if (my >= by + 20 && my < by + 37) {
                Loadout none = Loadout.EMPTY.withPerch(mine.perch());
                Wardrobe.setMine(none);
                Ui.play(SoundEvents.ENTITY_CAT_HISS, 1.6f, 0.2f);
                return true;
            }
        }
        if (tab == Tab.SETTINGS) {
            for (Toggle tg : toggles) {
                if (mx >= tg.x && mx < tg.x + tg.w && my >= tg.y && my < tg.y + 24) {
                    tg.flip.run();
                    ClientConfig.save();
                    Ui.play(SoundEvents.ENTITY_CAT_AMBIENT, tg.get.getAsBoolean() ? 1.6f : 1.1f, 0.3f);
                    return true;
                }
            }
            return false;
        ...
        for (Btn b : buttons) {
            if (mx >= b.x && mx < b.x + b.w && my >= b.y && my < b.y + b.h) {
                Ui.play(SoundEvents.ENTITY_CAT_AMBIENT, 1.5f, 0.3f);
                b.action.run();
                return true;
            }
        }
    ...
    private void equip(Card card) {
        ...
        if (card.cosmetic == null) {
            ...
            Ui.play(SoundEvents.ENTITY_CAT_AMBIENT, 1.0f, 0.2f);
            return;
        }
        ...
        if (!wearing) {
            Ui.play(slot == Slot.PET ? SoundEvents.ENTITY_CAT_PURREOW : SoundEvents.ENTITY_CAT_AMBIENT,
                    1.3f + Ui.rand() * 0.3f, 0.45f);
            petals.burst(card.x + CARD_W / 2f, card.y - scroll + CARD_H / 2f, 12);
        } else {
            Ui.play(SoundEvents.ENTITY_CAT_AMBIENT, 0.9f, 0.25f);
        }
    }
    ...
    public boolean mouseDragged(Click click, double dx, double dy) {
        if (dragging) {
            yaw -= (float) dx * 1.6f;
            pitch = MathHelper.clamp(pitch + (float) dy * 0.8f, -25, 35);
            yawVel = -(float) dx * 40;
            lastInteract = t;
            return true;
        }
    ...
    public static void devTab(int i) {
        if (MinecraftClient.getInstance().currentScreen instanceof WardrobeScreen w) {
