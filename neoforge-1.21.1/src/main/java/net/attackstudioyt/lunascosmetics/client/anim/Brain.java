...
public final class Brain {
    ...
    private Brain(long seed) {
        this.rng = RandomSource.create(seed * 31 + System.nanoTime());
    ...
    public void update(Inputs in) {
        ...
        dt = lastNanos < 0 ? 0 : Mth.clamp((now - lastNanos) / 1e9f, 0, 0.1f);
        lastNanos = now;
        if (dt == 0) {
            return;
        }
    ...
    public float stretch() {
        float s = time - wakeAt;
        return s < 0 || s > 1.2f ? 0 : Mth.sin(s / 1.2f * Mth.PI);
    }
