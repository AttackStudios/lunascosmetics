...
public final class Ui {
    ...
    public static ResourceLocation sprite(String name) {
        return ResourceLocation.fromNamespaceAndPath(LunasCosmetics.MOD_ID, "lunascosmetics/" + name);
    }

    public static void sprite(GuiGraphics ctx, String name, int x, int y, int w, int h) {
        RenderSystem.enableBlend();
        RenderSystem.defaultBlendFunc();
        ctx.blitSprite(sprite(name), x, y, w, h);
    }

    public static void sprite(GuiGraphics ctx, String name, int x, int y, int w, int h, int argb) {
        RenderSystem.enableBlend();
        RenderSystem.defaultBlendFunc();
        ctx.setColor(((argb >> 16) & 0xFF) / 255f, ((argb >> 8) & 0xFF) / 255f, (argb & 0xFF) / 255f,
                ((argb >>> 24) & 0xFF) / 255f);
        ctx.blitSprite(sprite(name), x, y, w, h);
    ...
    public static void round(GuiGraphics ctx, int x1, int y1, int x2, int y2, int r, int color) {
    ...
    public static void panel(GuiGraphics ctx, int x1, int y1, int x2, int y2, int r, int border, int fill) {
